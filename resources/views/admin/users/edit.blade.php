    ...
    <form method="post" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <select name="user_type" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-slate-250">
                ...
                <option value="E" {{ old('user_type', $user->user_type) == 'E' || old('user_type', $user->user_type) == 'F' ? 'selected' : '' }}>Funcionário</option>
        ...
        <div>
            ...
            @if($user->photo_url)
                <div class="mb-3 w-16 h-16 rounded-full overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                    <img src="{{ asset('storage/photos/' . $user->photo_url) }}" alt="Foto atual de {{ $user->name }}" class="w-full h-full object-cover">
                </div>
            @endif
