...
<div class="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-800 p-6 sm:p-8">
    <div class="flex items-center justify-between mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
        <div>
            <h2 class="text-xl font-bold text-slate-800 dark:text-white">Editar Categoria</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Modifique o nome ou a imagem de capa da categoria {{ $category->name }}.</p>
        </div>
    ...
    <form method="post" enctype="multipart/form-data" action="{{ route('admin.categories.update', $category) }}" class="space-y-6">
        @csrf
        ...
        <div>
            <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Imagem de Capa</label>
            @if($category->image_url)
                <div class="mb-4 flex items-center gap-3">
                    <img src="{{ asset('storage/categories/' . $category->image_url) }}" alt="{{ $category->name }}" 
                         class="w-20 h-20 object-cover rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm bg-slate-50 dark:bg-slate-800">
                    <div>
                        <span class="text-xs font-semibold text-slate-400 block">Imagem Atual</span>
                        <span class="text-xs text-slate-500 dark:text-slate-400 truncate max-w-xs block">{{ $category->image_url }}</span>
                    </div>
                </div>
            @endif
